...
file does not exist, Monit will try I</ect/monitrc> and a few
...
how much memory or cpu cycles is a process using.
...
/var/monit/etc> which will make Monit search for I<monitrc> in
I</var/monit/etc>
...
The I<-d interval> command-line switch runs Monit in daemon mode.
You must specify a numeric argument which is a polling interval
in seconds.
...
content, status, timeout, resource, checksum, fsflags, timestamp,
...
can be used in a I<check service>:
 ...
 check host FOO with address www.foo.bar
      if failed url 
         http://user:password@www.foo.bar:8080/login?querystring
         and content == 'j_security_check'
      then ...
...
Example 2: Do not run the check in the backup window on 
Sunday 0AM-1AM
...
I<credentials>, I<fips>, I<status> and I<failed>
